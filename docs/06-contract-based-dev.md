...
- **L2 Feature 契约**（版本级）：本版本作为整体对外的输入 / 输出 / 依赖 / 边界。
...
| 规则 | 说明 |
|------|------|
| 先契约后实现 | L1 / L2 在编码前确立；L3 在 400-build 规定，`dm-dev-step` 落地 |
...
| `skills/dm-dev-step.md` | 行为预期来自 400-build 行为契约；契约质量核查见 §2.5；失败面契约见 §2.7；结构化诊断见 07，指针 |
