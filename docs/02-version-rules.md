...
| `400-build.md` | Dev | 每个 Flow 怎么实现？怎么执行？ | 流内步骤、函数签名、Schema、异常策略 + 任务执行计划（预估、顺序、并行/串行） |
...
2. 400-build 再展开同一个 Transaction Flow 内部的具体步骤、接口、异常处理，末尾按执行顺序排列任务计划
```
...
`400-build.md` §6 是前文设计的执行视图。填充时以上方各 TF 详细设计为主要来源，以 `300-design.md` 补充依赖与顺序。
...
- `400-build.md` 模板用于快速起草伪代码、状态机、Schema、边界处理 + 任务执行计划。
