...
3. **步骤 3：模块内部实现（AI 主导，人做 Code Review）**
   仅把 Interface + 局部上下文喂给 AI，让其在模块内部完成实现，直至测试通过（执行粒度见 `docs/08`：单文件 / 单函数；陷入混乱时由人 `git reset --hard` 退回，见 `docs/08` §2.1）。
...
## 6. 跨层编码约定（Clean Code，契约视角）

> 本节是「跨层编码约定」的**唯一权威定义**；`docs/06-contract-based-dev.md` §3 指向此处 —— 该组约定**非契约质量维度**，故不在 06 展开。
