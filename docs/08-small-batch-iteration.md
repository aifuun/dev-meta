...
5. **UI 改动不写单测，但必须给出可复查的对照物**：AI **看不到渲染结果**，既无法自证视觉正确、人也无从验收 —— 视图 / 布局 / 样式改动须附**可复查证据**（Preview 截图 / 前后对比 / 真机验证记录）。视觉真值源是 **Design Token**（`docs/09` §5 第 8 条），AI 不得绕过 token 直写魔数；改动须可局部回滚（`docs/02` §6.2 S4）。
