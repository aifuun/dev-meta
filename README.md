...
[README.html](https://github.com/aifuun/dev-meta/blob/main/README.html) 是自包含的**单文件 HTML**：

- **查看**：双击用浏览器打开
- **发布**：推到 GitHub Pages 或任意静态托管直接访问
- **特点**：样式已内嵌，无需外部 CSS 或服务器

它由 `README.md` 生成，**不是手工维护的**。改动文档后重新生成：

```bash
pandoc README.md -f gfm -t html5 -s --metadata title=dev-meta -o README.html
```

> `README.md` 是唯一权威，`README.html` 是产物——不要手工编辑 HTML，改文档后重跑上面命令即可。
> 需要系统工具 `pandoc`（未安装则 `brew install pandoc`）。
