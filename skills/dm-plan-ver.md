...
400-build 末尾的执行顺序矩阵承载 **顺序、环节、依赖约束与验收约束（guard）**：
...
| guard | 该行须通过的防腐契约编号（`GUARD-01,03`…）；每个编号须能在 §1.4 找到对应行 |
| 备注 | 前置依赖、并行提示、风险标记 |
...
3.5 **开 TF Issue 前的契约门禁自检（Gate）**
   生成 Issue（即把 TF 拆成可执行单元）之前，确认 `400-build.md` 已落地 06/07/08 基线，否则退回补写、不开 Issue：
   - **06 契约式开发**：含 L1/L2/L3 行为契约、执行顺序矩阵带环节、失败面不静默、契约四要素标注（归属/方向/不变性/真值来源 + 域-序号编号，见 `docs/06-contract-based-dev.md` §2.5/§2.6/§2.7/§3）
