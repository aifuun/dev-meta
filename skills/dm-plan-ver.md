...
| 文档 | 回答的问题 | 前置依赖 |
|------|-----------|----------|
| `200-spec.md` | 交付什么？怎样算完成？（背景 + 用户旅程 + 核心业务场景 + 架构锚点 + 验收） | — |
...
| 触发点 | Grill 级别 | 提问范围 |
|--------|-----------|----------|
| 创建 `200-spec.md` 前 | 需求级 | **本项工作的说明（背景）**（这项工作是什么、为什么现在做、给谁用）、**用户旅程**（主路径 + 关键失败 / 降级 / 取消分支）、范围边界、完成标准、降级策略、明确排除项、**架构锚点五维**（触及哪些分层 / 模块 / 门面、**受影响契约及是否阻塞编码** / 对外 API 变更形态） |
| 创建 `300-design.md` 前 | 架构级（最关键） | 数据流、模块边界、异常处理、兼容性、技术选型 |

> 📌 **需求级 grill 的边界**：背景与旅程只问「这项工作是干什么的、用户怎么走通、在哪分叉」；
> 访谈得到的术语请在写入 `## 背景（小白向）` 时用白话解释（规范见 `docs/02` §3.4）。
> 架构锚点只问「本版本动到架构哪几处」「受影响契约是否阻塞编码」，
...
1. **创建版本文档** `docs/versions/v1.3-export/`（四件套，顺序见「核心概念」；`200-spec.md` 与 `300-design.md` 产出前分别执行需求级/架构级 grill，见「核心概念」Grill 决策收敛）
   - **需求级 grill**（创建 `200-spec.md` 前）：围绕本项工作的说明（背景）、用户旅程（主路径 + 关键分支 / 降级）、范围边界、完成标准、降级策略、排除项提问，回答沉淀进 `200-spec.md`
   - 创建 `200-spec.md`：**开篇两个无编号块必须齐全** —— `## 背景（小白向）`（大白话，术语首次出现须紧跟解释）+ `## 用户旅程（用例）`（mermaid 主图 + 分支表），位于一句话场景之后、`## 1. 架构锚点` 之前（结构见 `docs/02` §4）
...
| `200-spec` 开篇须有**两个无编号前言块**：`## 背景（小白向）`、`## 用户旅程（用例）`；是唯一允许的无编号章节，新增须先改规范 | 02-version-rules.md §3.4 / §4 |
| 背景块 ≤ 150 字、零前置知识可读、术语首次出现须紧跟白话解释；只写「是什么 / 为什么」，不写技术方案 | 02-version-rules.md §3.4 / §4 |
| 旅程块 = mermaid 主图（≤ 12 节点，用户动作命名）+ 分支表（≤ 5 行，主路径 + 关键失败 / 降级 / 取消）；细碎异常留 300-design | 02-version-rules.md §3.4 / §4 |
...
- [ ] `200-spec` **背景（小白向）已写**（零前置知识可读，术语首次出现已紧跟白话解释，≤ 150 字）
- [ ] `200-spec` **用户旅程已画**（mermaid 主路径 + 关键失败 / 降级 / 取消分支齐全，且与架构锚点无重复）
...
| `~/.dev-meta/templates/versions/vX.Y-<slug>/200-spec.md` | `templates/versions/vX.Y-<slug>/200-spec.md`（发布后） | 规格模板（背景 + 用户旅程 + 核心业务场景 + 架构锚点 + 验收 + DoD） |
