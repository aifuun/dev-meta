...
| 实现降级级 | 并发边界 / 依赖集成 / 测试策略 / 契约质量 / **可观测性盲区** | 高并发边界？外部依赖如何集成？测试怎么覆盖？错误/幂等/兼容/不变量契约是否齐全？**可观测性盲区：错误/降级路径有无结构化诊断（07 §2.1/§3）？有无静默吞错 / 裸露日志？高开销节点是否含耗时+资源指标（07 §2.3）？**（契约规范见 `docs/06-contract-based-dev.md` §3 / §10，可观测性见 `docs/07-observability-driven-dev.md`） |
