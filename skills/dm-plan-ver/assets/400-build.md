...
> ID 用 `GUARD-01` 递增（沿用 `docs/06` §4 域-序号），**与契约层级 L1 / L2 / L3 无关**。
...
> 跳过声明唯一合法格式：**执行态**填 `⏭️ SKIPPED`，末列写「跳过：\<理由\>」。
> 「执行态」是设计决定（执行 / SKIPPED）；**进度状态**（⬜/🔄/✅）只写 `500-schedule.md`。
...
> 对**每个执行态为「执行」**的 Step 展开一小节（`⏭️ SKIPPED` 的不展开）。
> **轨迹**：S4 / S5 接线时须**同时接好轨迹观测点**并做完整链路集成验证；S6 须做**轨迹资产核对与验证**（必做）——每观测点须给判别量，验证落在集成 / E2E 层（`docs/02` §6.2、`docs/07` §3.2）。
...
> 仅对算法类 / 有隐性契约（幂等、并发去重、异常分支、降级取舍）的函数填写；薄胶水 / CRUD 可省略。本表是「行为规定」而非「测试实现」，`dm-dev-step` 落地时据此生成真实单测。分工见 `docs/06-contract-based-dev.md` §10。
