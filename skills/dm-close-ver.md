---
name: dm-close-ver
description: 关闭版本时使用：就绪审计、保留历史的 merge（不用 squash）、关闭 版本 Issue、清理分支、打 annotated tag 并推送。触发于「关闭版本」。
---
...
| **兜底核对** 版本 Issue 已关闭 + 清理分支、打 annotated tag 并推送 | ✅ 本 skill |
...
dm-close-ver 是 dm-plan-ver **阶段 3**（版本收尾）的独立承接者。dm-plan-ver 负责版本规划与启动（四件套、分支、PR、版本 Issue）；版本进入收尾阶段后由本 skill 接管。

```
dm-plan-ver (开版本)
  └── 阶段 2: Step 开发 —— 委托 dm-dev-step 全程承接
        dm-dev-step
...
- **前置条件**：版本分支 `feature/vX.Y-<slug>` 已存在，Step 开发已完成或明确延期
...
用 **merge commit**（`--no-ff`）合并版本分支到 main，**不使用 squash**——保留每个 Step commit 的原始历史，便于追溯与回滚。
...
| 场景 | 识别信号 | 处理 |
|------|----------|------|
| **Issue 漏关** | 版本合并后仍有 open 版本 Issue | 逐个核对 → 关闭或标记 `[DEFERRED]` |
...
| **误用 squash** | 合并后 Step 历史被压平 | 确认分支已推送后，用 merge commit 方式重建或记录回滚点 |
...
- [ ] 所有 版本 Issue 已关闭，或明确标记 `[DEFERRED]` 并备注原因
- [ ] 已合并的版本分支已删除
- [ ] annotated tag 已创建并**推送到远端**
- [ ] roadmap 已标记该版本交付
- [ ] 关闭报告从 `500-schedule.md` 执行记录**汇总**而成，未另行重写过程
...
AI:  1. 就绪性审计:
        - Issue #21 [v1.5] 登录表单 → closed ✅（版本 Issue 唯一）
        - 500-schedule.md 全部 ✅
        - git status 干净
     ...
     3. 合并(保留历史):
        ...
        git push origin main
     4. 关闭 版本 Issue: 确认 #21/#22 已自动关闭(Closes footer)
