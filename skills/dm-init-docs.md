...
| 项目层 | `./CODEBUDDY.md` | 仅两块：**规范来源表**（dev-meta 仓库地址 + 采用版本）+ **本项目例外**。不复制规范正文 |
...
基于模板在项目根目录创建 `./CODEBUDDY.md`，填写 dev-meta 仓库地址与采用版本，并留「本项目例外」待填。作用：项目与 dev-meta 的版本绑定 + 例外项记录；通用规范由 `~/.codebuddy/CODEBUDDY.md` 全局加载，不重复。
